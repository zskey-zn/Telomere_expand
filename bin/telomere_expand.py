...
parser.add_argument('--category',help='species category',default=True)
...
#info_file = "expand.info"
#genome_file = "../00.gapcloser_merge/gap_free.fa"
#candidate_file = "../00.gapcloser_merge/candidate_contig.fa"
info=[]
...
""".format("N"*100,candidate_file,tail_content[0][2],tail_content[0][4],int(tail_content[0][4]/100-1)*100,int(tail_content[0][4])+20000)
    ...
    #gapcloser
    content+="""/public/frasergen/PUB/software/TGS-GapCloser/TGS-GapCloser_v1.0.1/TGS-GapCloser.sh --scaff telomere_without.fa --reads candicate.fa --minmap_arg '-x {}' --min_idy 0.2 --min_match {} --thread 20  --output gapclose --ne >pipe.log 2>pipe.err
echo end at `date`
touch work.sh.finish
""".format(map_para,min_match)
    f.write(content)
    f.close()
    os.chdir("..")
